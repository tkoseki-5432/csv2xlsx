...
#get xlsx file list
xlsxfilelist = list(path_obj.glob("*.xlsx"))

if xlsxfilelist.count("testresult.xlsx")!=0:

    #test result excel open
    wb2 = xl.load_workbook(str(xlsxfilelist[xlsxfilelist.index("testresult.xlsx")]))
...
for listnum in  range(len(csvfile)):
    ...
    if ProdactSirialnum.count(name[15:23])==0:
        #write ProdactSirialnum to xlsx
        ProdactSirialnum.append(name[15:23])
        num = len(ProdactSirialnum)
        sheet1.cell(row=1, column=num+1).value = ProdactSirialnum[num-1] 
        sheet2.cell(row=1, column=num+1).value = ProdactSirialnum[num-1] 
     
    else:
        num=ProdactSirialnum.index(name[15:23])+1
